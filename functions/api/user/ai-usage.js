...
function publicRoute(row={}){
 const premium=String(row.access_class||'')==='premium';
 if(!premium)return row;
 return {feature:row.feature,access_class:'premium',decision:row.decision,reason:row.decision==='used'?'premium_entitlement':'premium_policy',estimated_cost_usd:Number(row.estimated_cost_usd||0),created_at:row.created_at,label:row.feature==='video'?'Premium Video':'Premium Intelligence'};
}
export async function onRequestGet({request,env}){
 ...
 return json({ok:true,plan:a.user.plan||'free',walletBalanceUsd:wallet,entitlements,recentRouting:(recent.results||[]).map(publicRoute)});
}
