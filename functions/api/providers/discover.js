...
export async function onRequestPost({request,env}){
 ...
 try{
  ...
  const key=await decryptStoredKey(env,row);if(!key)return json({ok:false,error:'No API key stored for this provider'},400);
  let models=[];
  if(provider==='openrouter'){
   const b=await fetchJson('https://openrouter.ai/api/v1/models',{headers:{Authorization:`Bearer ${key}`}});
   models=(b.data||[]).map(m=>({id:m.id,name:m.name||m.id,qualifiedFree:openRouterFree(m),source:'provider-pricing'}));
  }else if(provider==='groq'){
   const b=await fetchJson('https://api.groq.com/openai/v1/models',{headers:{Authorization:`Bearer ${key}`}});
   models=(b.data||[]).map(m=>({id:m.id,name:m.id,qualifiedFree:false,source:'discovered-only'}));
  }else if(provider==='gemini'){
   const b=await fetchJson(`https://generativelanguage.googleapis.com/v1beta/models?key=${encodeURIComponent(key)}`);
   models=(b.models||[]).filter(m=>(m.supportedGenerationMethods||[]).includes('generateContent')).map(m=>({id:String(m.name||'').replace(/^models\//,''),name:m.displayName||m.name,qualifiedFree:false,source:'discovered-only'}));
  }else if(provider==='deepseek'){
   const base=(row.endpoint||'https://api.deepseek.com').replace(/\/$/,'');
   const b=await fetchJson(`${base}/models`,{headers:{Authorization:`Bearer ${key}`}});
   models=(b.data||[]).map(m=>({id:m.id,name:m.id,qualifiedFree:false,source:'discovered-only'}));
