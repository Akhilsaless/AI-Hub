(()=>{
  ...
  let recognition=null,listening=false,autoSpeak=localStorage.hopeVoice==='1',lastSpoken='',voiceError=false;
  // The legacy page owns a second speech path. Turn that path off once, then let HOPE 6 own voice exclusively.
  if(autoSpeak){try{voiceBtn.click()}catch{}localStorage.hopeVoice='1'}
  ...
  function startListening(){
    ...
    recognition=new SpeechRecognition();recognition.lang=navigator.language||'en-IN';recognition.interimResults=true;recognition.continuous=false;recognition.maxAlternatives=1;let finalText='';voiceError=false;
    ...
    recognition.onerror=e=>{voiceError=true;listening=false;micBtn.textContent='🎙';setState(e.error==='not-allowed'?'Microphone permission is blocked':'I couldn’t hear that clearly')};
    recognition.onend=()=>{listening=false;micBtn.textContent='🎙';micBtn.title='Talk to HOPE';if(voiceError)return;const text=(finalText||inputBox.value||'').trim();if(text){setState('Sending voice message…');setTimeout(()=>sendBtn.click(),120)}else setState('Ready')};
    try{recognition.start()}catch{voiceError=true;setState('Microphone could not start')}
  }
